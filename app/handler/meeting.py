...
class MeetingHandler:
    ...
    def validateMeetingInput(self, ccode, starttime, endtime, cdays):
        ...
        if starttime_dt >= endtime_dt or starttime_dt == endtime_dt:
            return (
                jsonify(
                    InsertStatus="Invalid time range, starttime is the same or more than endtime"
                ),
                400,
            )
        ...
        if cdays == "MJ" and (
            starttime_dt >= datetime.strptime("10:15", "%H:%M")
            and endtime_dt <= datetime.strptime("12:30", "%H:%M")
        ):
            return (
                jsonify(
                    InsertStatus="Invalid time range for MJ meetings, 'Hora Universal'"
                ),
                400,
            )

        if cdays == "LWV" and not (
            endtime_dt - starttime_dt == timedelta(hours=0, minutes=50, seconds=0)
        ):
            return jsonify(InsertStatus="Invalid time range for LMV meetings"), 400

        if cdays == "MJ" and not (
            endtime_dt - starttime_dt == timedelta(hours=1, minutes=15, seconds=0)
        ):
    ...
    def insertMeeting(self, meeting_json):
        ...
        jsonify_error, error = self.validateMeetingInput(
        ...
        if mid_temp:
            return (
                jsonify(InsertStatus=f"Duplicate Meeting, meeting id: {mid_temp[0]}"),
                404,
            )
        ...
        endtime_dt = datetime.strptime(
            endtime.split(":")[0] + ":" + endtime.split(":")[1], "%H:%M"
        )
        ...
        if (
            starttime_dt >= timedelta_10_15
            and starttime_dt < timedelta_12_30
            and cdays == "MJ"
        ):
            ...
            mid_temp = dao.checkMeetingDuplicate(
                ccode, starttime_temp, endtime_temp, cdays
            )
            if mid_temp:
                return (
                    jsonify(
            ...
            endtime_dt < timedelta_12_30
            and endtime_dt > timedelta_10_15
            and cdays == "MJ"
        ):
            ...
            if mid_temp:
                return (
                    jsonify(
                        InsertStatus=f"Meeting conflict with meeting at 9:00am, meeting id: {mid_temp[0]}"
                    ),
                    400,
                )
        ...
        mid = dao.insertMeeting(
            ccode,
            starttime,
            endtime,
            cdays,
            delta_time_to_left=delta_time_to_left_str,
            delta_time_to_right=delta_time_to_right_str,
        )
        mid_To_Delete = dao.deleteAllMeetingsWithInvalidTime()
        if mid_To_Delete is not None:
            return (
                jsonify(
                    InsertStatus="OK, This insert delete the meetings with 'ids' %s"
                    % (mid_To_Delete)
                ),
                200,
            )
    ...
    def updateMeetingByMid(self, mid, meeting_json):
        ...
        if (
            ...
            mid_temp = dao.checkMeetingDuplicate(
                ccode,
                str(starttime_dt + delta_time_to_right),
            ...
            mid_temp = dao.checkMeetingDuplicate(
                ccode,
                str(starttime_dt - delta_time_to_left),
