...
st.markdown(
    """
    Welcome to this page! Here, you can explore a comprehensive overview of academic metrics 
    through visually engaging and interactive charts. Dive into data insights such as 
    meetings with the highest number of sections, classes with the most prerequisites, 
    the least frequently offered classes, and annual trends in total sections.
    """
)
...
if st.session_state.get("login"):
    ...
    with top_five_meetings_with_most_sections_container:
        ...
        response = requests.post(
            "https://rumad-db-5dd7ab118ab8.herokuapp.com/segmentation_fault/most/meeting"
        )
        # if response.status_code == 200:
        data = response.json()
        df = pd.json_normalize(data)
        df["normalized_section_count"] = (
            df["section_count"] - df["section_count"].min()
        ) / (df["section_count"].max() - df["section_count"].min())
    ...
    with top_three_classes_as_prerequisite_container:
        ...
        response = requests.post(
            "https://rumad-db-5dd7ab118ab8.herokuapp.com/segmentation_fault/most/prerequisite"
        )
        if response.status_code == 200:
            data = response.json()
            df = pd.json_normalize(data)
            df["normalized_prerequisite_count"] = (
                df["prerequisite_classes"] - df["prerequisite_classes"].min()
            ) / (df["prerequisite_classes"].max() - df["prerequisite_classes"].min())
            df["colors"] = generate_green_shades(
                "#327136", df["normalized_prerequisite_count"]
            )
            ...
            fig = px.bar(
                ...
                labels={
                    "prerequisite_classes": "Number of prerequisite",
                    "cid": "Class ID",
                },
            )
            fig.update_traces(marker_color="#327136")
    ...
    with top_three_classes_offered_least_container:
        ...
        st.divider()
        response = requests.post(
            "https://rumad-db-5dd7ab118ab8.herokuapp.com/segmentation_fault/least/classes"
        )
        if response.status_code == 200:
            data = response.json()
            df = pd.json_normalize(data)
            ...
            ) / (df["class_count"].max() - df["class_count"].min() + epsilon)
    ...
    with total_sections_per_year_container:
        ...
        st.divider()
        response = requests.post(
            "https://rumad-db-5dd7ab118ab8.herokuapp.com/segmentation_fault/section/year"
        )
        data = response.json()
        df = pd.json_normalize(data)

        epsilon = 1e-10
        df["normalized_sections"] = (df["sections"] - df["sections"].min()) / (
            df["sections"].max() - df["sections"].min() + epsilon
        )
